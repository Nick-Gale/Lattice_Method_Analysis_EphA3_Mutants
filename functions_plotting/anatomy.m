...
%--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    save_dir = strcat(dict.directory, sprintf('figure_anatomy_ID(EphA3Ki, Ratio, B2, Repeat): (%0.2f, %0.2f, %d, %d).png', id(1), id(2), id(3), id(4)));
...
%--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    %subplot one, colour coded retinal positions
    ...
    subplot4_x = collicular_positions_RC(ml_slice_indexes);
    subplot4_y = average_retinal_projection(ml_slice_indexes);
    ...
    subplot5_EphA3_x = average_collicular_projection_RC(intersect(ml_slice_indexes, ephA3_indexes));
    subplot5_EphA3_y = retinal_positions_NT(intersect(ml_slice_indexes, ephA3_indexes));

    subplot5_WT_x = average_collicular_projection_RC(intersect(ml_slice_indexes, WT_indexes));
    subplot5_WT_y = retinal_positions_NT(intersect(ml_slice_indexes, WT_indexes));
...
%figure 2
    ...
    if ~isempty(subplot2_ephA3_y)
        ...
        line([0 1], [min(subplot2_ephA3_y) min(subplot2_ephA3_y)], 'LineStyle', '--', 'Color', epha3_colour);
    else
        gradients1 = plot(subplot2_WT_x, subplot2_WT_x, 'Color', epha3_colour);
    end
    hold on
    ...
    %line(subplot2_ephrinA_x, subplot2_ephrinA_y)
...
%figure 4
    subplot(7,1,5)
    scatter(subplot4_x, subplot4_y, 'filled', 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'k')
    alpha(transparency)
